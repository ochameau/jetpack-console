...
var EXPORTED_SYMBOLS = ["HUDService", "ConsoleUtils", "HeadsUpDisplay"];
...
HeadsUpDisplay.prototype = {
  ...
  makeHUDNodes: function HUD_makeHUDNodes()
  {
    ...
    ConsoleUtils.outputNode = this.outputNode;
...
let ConsoleAPIObserver = {
  ...
  observe: function CAO_observe(aMessage, aTopic, aData)
  {
    ...
    else if (aTopic == "jetpack-api-log-event") {
      aMessage = aMessage.wrappedJSObject;

      HUDService.logConsoleAPIMessage("hud_browserconsole", aMessage);
    }
...
JSTerm.prototype = {
  ...
  createSandbox: function JST_setupSandbox()
  {
    ...
    this.sandbox = new Cu.Sandbox(principal,
  ...
  {
Cu.reportError("complete: "+type);
    let inputNode = this.inputNode;
    let inputValue = inputNode.value;
    // If the inputNode has no value, then don't try to complete on it.
    if (!inputValue) {
    Cu.reportError("no iv");
      this.clearCompletion();
      return false;
    }
    ...
    if (!this.lastCompletion || this.lastCompletion.value != inputValue) {
      let properties = this.propertyProvider(this.sandbox, inputValue);
      if (!properties || !properties.matches.length) {
      Cu.reportError("properties");
        this.clearCompletion();
        return false;
      }
...
ConsoleUtils = {
  ...
  outputMessageNode: function ConsoleUtils_outputMessageNode(aNode, aHUDId) {
    ...
    let outputNode = this.outputNode; //HUDService.hudReferences[aHUDId].outputNode;
